...
func TestRetryDelayClassification(t *testing.T) {
	...
	t.Run("429 honors Retry-After seconds", func(t *testing.T) {
		d, ok := p.retryDelay(httpErr(429, http.Header{"Retry-After": {"7"}}), 1)
	...
	t.Run("403 secondary limit via X-RateLimit-Reset", func(t *testing.T) {
		...
		d, ok := p.retryDelay(httpErr(403, h), 1)
	...
	t.Run("ordinary 403 is not retryable", func(t *testing.T) {
		if _, ok := p.retryDelay(httpErr(403, nil), 1); ok {
			t.Error("a permission 403 should not be retried")
		}
	})

	t.Run("5xx retries with backoff", func(t *testing.T) {
		d, ok := p.retryDelay(httpErr(503, nil), 1)
		if !ok || d != baseBackoff {
	...
	t.Run("404 is not retryable", func(t *testing.T) {
		if _, ok := p.retryDelay(httpErr(404, nil), 1); ok {
			t.Error("404 should not be retried")
		}
	})

	t.Run("transport error retries", func(t *testing.T) {
		if _, ok := p.retryDelay(errors.New("connection reset"), 1); !ok {
